...
int main(int argc, char *argv[]) {
  try {
    ...
    std::string nnet3_rxfilename = po.GetArg(1), fst_rxfilename = po.GetArg(2),
                wav_rspecifier = po.GetArg(3), output_path = po.GetArg(4);
    ...
    for (int iter = 0; iter < iterations; iter++) {
      ...
      // Okay, so a single thread reads one at a time... Is it ever backpressured?
      for (; !wav_reader.Done(); wav_reader.Next()) {
        std::string utt = wav_reader.Key();
        std::string key = utt;
        if (iter > 0) key = std::to_string(iter) + "-" + key;
        std::string write_path = output_path + "/" + key + ".ctm";
        if (!access(write_path.c_str(), F_OK)) {
          num_task_submitted++;
          continue;
        }
        ...
        SegmentedResultsCallback segmented_callback =
            [&output_path, key, &word_syms](SegmentedLatticeCallbackParams &params) {
              KALDI_LOG << "GALVEZ:output key=" << key;
              Output ctm_output(output_path + "/" + key + ".ctm", /*binary=*/false);
              MergeSegmentsToCTMOutput(params.results, key,
                                       ctm_output.Stream(), word_syms);
            };

        int result_type = 0;
        result_type |= CudaPipelineResult::RESULT_TYPE_CTM;
