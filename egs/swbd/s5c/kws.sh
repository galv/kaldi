...
mfccdir=`pwd`/mfcc
modelDir=tri2
graphDir=tri2/graph_nosp_sw1_tg/
min_lmwt=1
max_lmwt=20
step_lmwt=1
...
steps/nnet2/decode.sh --nj $jobs --cmd "$decode_cmd" \
...
for l in `seq 1 $jobs`; do 
   if [ -f exp/$modelDir/decode_workfit/lat.$l.gz ]; then
     ...
     wait
     (
       outputFileName=`head -$l data/workfit/wav.scp | tail -1 | cut -f3 -d" " | cut -f1-4 -d"."`".kaldi.swbd.ctm"
       cat exp/$modelDir/decode_workfit/path_*.$l.ctm | cut -f3-100 -d" " | sort -gu > $outputFileName
