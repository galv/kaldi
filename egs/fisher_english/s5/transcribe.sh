...
for l in `seq 1 $jobs`; do 
   (
     if [ -f exp/$modelDir/decode_workfit/lat.$l.gz ]; then
       $kaldiHome/src/latbin/lattice-1best "ark:gunzip -c exp/$modelDir/decode_workfit/lat.$l.gz|" ark:- | \
         $kaldiHome/src/latbin/lattice-align-words exp/$graphDir/phones/word_boundary.int exp/$modelDir/final.mdl ark:- ark:- | \
         ~/kaldi/src/latbin/lattice-to-ctm-conf --decode-mbr=true ark:- -  | \
         utils/int2sym.pl -f 5 exp/$graphDir/words.txt > exp/$modelDir/decode_workfit/bestPath.$l.ctm || exit 1;
         
       outputFileName=`head -$l data/workfit/wav.scp | tail -1 | cut -f3 -d" " | cut -f1-4 -d"."`".kaldi.fisher.transcription.ctm"
       cat exp/$modelDir/decode_workfit/bestPath.$l.ctm | cut -f3-100 -d" " | sort -gu > $outputFileName
       echo "Transcription saved to: "$outputFileName             
     fi;
   ) &
done
wait
